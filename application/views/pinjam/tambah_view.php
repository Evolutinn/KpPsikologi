...
$isi = array(
    'buku_id' => 123,
    'jml' => 0 // Ganti nilai ini sesuai kebutuhan Anda
);
?>
						...
						<tbody>
							...
							foreach ($buku->result_array() as $isi) { ?>
								<tr>
									...
									<?php if ($isi['jml'] == 0): ?>
    <button class="btn btn-danger" id="" data_id="<?= $isi['buku_id']; ?>">
        <i class="fa fa-times"></i> Habis
    </button>
<?php else: ?>
							...
							foreach ($user as $isi) {
								if ($isi['level'] == 'Anggota') {
									...
									<tr>
										...
										<td style="width:20%;">
											<button class="btn btn-primary" id="Select_File1"
												data_id="<?= $isi['anggota_id']; ?>">
												<i class="fa fa-check"> </i> Pilih
											</button>
										</td>
									</tr>
